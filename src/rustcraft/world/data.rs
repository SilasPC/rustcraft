...
impl WorldData {
    ...
    pub fn refresh(&mut self, reg: &Registry) {
        ...
        cc.retain(|x| self.blocks.chunk_at(*x).map(Chunk::renderable).unwrap_or(false));
        // println!("refresh {:?}",cc);
        for cp in &cc {
            calc_light(*cp, self);
        }
            ...
            for x in -1..=1 {
                for y in -1..=1 {
                    for z in -1..=1 {
                        ...
                        if !self.blocks.chunks.get(&p.into()).unwrap().renderable() || meshed.contains(&p) {continue}
                        let (m1,m2) = (meshing::make_mesh(p.into(), self, reg));
                        let c = self.blocks.chunks.get_mut(&p.into()).unwrap();
                        {
